  ...
  private void run(String[] arguments) throws Exception {
    ...
    for (int i = 0; i < A.length; i++) {
      for (int j = 0; j < A.length; j++) {
        if (A[i]>A[j]) outerCount++;
      }
    }

    long POWER = (long) (Math.pow(10, 9) + 7);
    
    long ans = (innerCount * (long) K) % POWER;
    long outerTimes = ((K * (K -1)) / 2) % POWER;
    ans += (outerCount * outerTimes)%POWER;

    out.println(ans%POWER);
  }
