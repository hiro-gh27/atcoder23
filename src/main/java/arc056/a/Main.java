...
public class Main {
  ...
  private void run(String[] arguments) throws Exception {
    ...
    long unitPrice = A * K;
    long setPrice = ((K / L) + 1) * B;

    long numberOfSet = (K / L);
    long combinationPrice = ((numberOfSet) * B) + (K-numberOfSet*L) * A;

    long ans = Math.min(unitPrice, setPrice);
    ans = Math.min(ans, combinationPrice);
    out.println(ans);
