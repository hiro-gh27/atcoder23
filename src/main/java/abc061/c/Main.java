...
public class Main {
  ...
  private void run(String[] arguments) throws Exception {
    ...
    for (int i = 0; i < N; i++) {
      ...
      inputs.put(number, inputs.getOrDefault(number, 0)+repeatCount);
    }

    int index = 0;
    long[] keys = new long[inputs.keySet().size()];
    long[] accValues = new long[inputs.keySet().size()];
    long acc = 0;
    for (Entry<Integer, Integer> input : inputs.entrySet()) {
      acc += input.getValue();
      keys[index] = input.getKey();
      accValues[index] = acc;
      index++;
    }
