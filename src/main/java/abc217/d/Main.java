...
public class Main {
  ...
  private void run(String[] arguments) throws Exception {
    ...
    for (int i = 0; i < numberOfQuery; i++) {
      ...
      int queryArgs = sc.nextInt();
      if (queryType == 1){
        treeSet.add(queryArgs);
      }else{
        int low = treeSet.lower(queryArgs);
        int hight = treeSet.higher(queryArgs);
        int currentLen = hight - low;          
        out.println(currentLen);
      }
    }
