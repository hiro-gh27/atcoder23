...
public class Main {
  ...
  boolean isPalindrome(String a, String b){
    char[] joined = (a + b).toCharArray();
    int low = 0;
    int hight = joined.length-1;
    while (low < hight){
      if (joined[low] != joined[hight]){
        return false;
      }
      low ++;
      hight --;
  ...
  private void run(String[] arguments) throws Exception {
    ...
    for (int i = 0; i < wordsList.size(); i++) {
      for (int j = 0; j < wordsList.size(); j++) {
        if (i == j) continue;
        String l = wordsList.get(i);
        String r = wordsList.get(j);
        if (isPalindrome(l,r)){
          out.println("Yes");
          return;
        }
