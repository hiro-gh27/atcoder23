...
public class Main {
  ...
  private void run(String[] arguments) throws Exception {
    ...
    HashMap<Long, Long> workers = new HashMap<>();
    for (int i = 0; i < N; i++) {
      long w = sc.nextLong();
      long x = sc.nextLong();
      workers.put(x, workers.getOrDefault(x, 0L)+w);
    }

    long ans = 0;
    for (int i = 0; i < 25; i++) {
      long accept = 0;
      for (long j = i; j < i+9; j++) {
        accept += workers.getOrDefault(j%24, 0L);
      }
      ans = Math.max(accept, ans);
    }
    out.println(ans);
