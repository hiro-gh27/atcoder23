...
public class Main {
  ...
  private void run(String[] arguments) throws Exception {
    ...
    for (int i = 0; i < expectedValues.length; i++) {
      ...
      expectedValues[i] = (1.0 + value) / 2.0;
    }

    double windowValue = 0;
    for (int i = 0; i < K; i++) {
      windowValue += expectedValues[i];
    }

    double maxWindowValue = windowValue;
    for (int i = 1; i <= expectedValues.length - K; i++) {
      windowValue -= expectedValues[i -1];
      windowValue += expectedValues[i + K - 1];
      maxWindowValue = Math.max(maxWindowValue, windowValue);
    }
    out.println(maxWindowValue);
  }
