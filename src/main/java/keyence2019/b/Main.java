...
public class Main {
  ...
  private String customToString(char[] inputs){
  ...
  private void run(String[] arguments) throws Exception {
    ...
    for (int i = 0; i < inputs.length; i++) {
      ...
      for (int j = i; j < inputs.length; j++) {
        copied[j] = '.';
        String current = customToString(copied);
        if ("keyence".equals(current)){
          out.println("YES");
          return;
        }
      }
    ...
    String ans = customToString(inputs).equals("keyence") ? "YES" : "NO";
    out.println(ans);
  }
